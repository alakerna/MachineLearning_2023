...
public class HMM extends Model {
	...
	public void fit(DataLoader trainLoader) {
		...
		for(int k=0; k< trainLoader._numDocs; k++){
			ArrayList<String> wordDoc = trainLoader.getDoc(k);
			ListIterator<String> wordIt= wordDoc.listIterator();

			ArrayList<State> stateDoc = trainLoader.getStates(k);
		...
		for (State srcState : State.values()) {
			...
			double denom = _stateVisitCounts[srcId];
			for (State destState : State.values()) {
				
				Integer destId = destState.getID();
				double num = _state2StateVisitCounts[srcId][destId];
				double value = num/denom;

				if( Math.abs(num) < this.threshold ){
		...
		for(State s : State.values()) {
			Integer sId = s.getID();
			double denom = _stateVisitCounts[sId];
			for(int wId=0; wId< DataLoader.numWords ; wId++){
				double num = _obsVisitCounts[sId][wId];
				double value = num/denom;
