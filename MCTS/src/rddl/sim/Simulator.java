...
public class Simulator {
	...
	public Result run(Policy p, StateViz v, long rand_seed) throws EvalException {
		...
		for (int t = 0; t < _i._nHorizon; t++) {
			...
			// Check how much time is allocated per action selection (total sum should not change)
			if (p instanceof MCTS | 
				p instanceof MySelectionMCTS | 
				p instanceof MyBackpropMCTS | 
				p instanceof CompetitiveMCTS |
				p instanceof RolloutPolicy) {
				long timePerStep = ((MCTS)p).getTimePerAction();
				totalPlanTime += timePerStep;
				if (totalPlanTime > MCTS.TIMEOUT * Math.pow(10, 6) + 10000) {
					System.err.println(String.format("TIMEOUT illegally modified! The total sum should be below %d. Exiting...", MCTS.TIMEOUT));
					System.exit(1);
				}	
			}
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length >= 5)
			rand_seed_sim = Integer.valueOf(args[4]);
		int rand_seed_policy = (int)System.currentTimeMillis(); // 123456
		if (args.length >= 6)
			rand_seed_policy = Integer.valueOf(args[5]);
